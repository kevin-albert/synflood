 ...
 */
void print_usage (FILE *fout, char *execname) {
    fprintf (fout, "usage: %s -h hostname [-p port] [-q]\n", execname);
}
...
/**
 * execute the attack 
 * each packet is generated with a random source ip, port, id.
 * will run until the "stop" flag is set in another thread (via the signal 
 * handler)
...
/**
 * executed when a termination signal is received. sets the stop flag
 */
void onsignal (int sig) {
    if (!q) {
        printf ("shutting down\n");
    }
    stop = 1;   // signal the main thread to stop
}
